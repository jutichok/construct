...
function getProduct($pageNow,$strSort,$strOrder){
	$str="";
	$uid="";
	$sql = "SELECT colid, colname, t.coltypename as coltypename FROM `tcollection` 
		inner join tcoltype t on t.coltypeid = coltype";
	...
	while($uid = mysql_fetch_array($qry))
	{
		...
		$str.=("<colid>".$uid["colid"]."</colid>");
		$str.=("<colname>".$uid["colname"]."</colname>");
